...
class PostDamagePreTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        v_PostDamagePostTech  = self.v_PostDamagePostTech_nn( tf.concat([logK, Z, Y,  λ3, A_g_prime_prime *tf.ones(tf.shape(Y), dtype=Y.dtype) ,logξ, logξ], 1) )

        g_l_prime_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamagePostTech - v), -700.0, 350.0) )
        ...
        if self.use_intermediate_tech_jump:
            ...
            g_l_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamageIntermTech - v), -700.0, 350.0) )
    ...
    def train(self):
        ...
        if NBER_folder is not None:
            load_weights_cast(self.v_nn,   self.params['v_nn_config'],   NBER_folder + "/pre_tech_post_damage/v_nn_checkpoint_pre_tech_post_damage", n_inputs)
            load_weights_cast(self.i_g_nn, self.params['i_g_nn_config'], NBER_folder + "/pre_tech_post_damage/i_g_nn_checkpoint_pre_tech_post_damage", n_inputs)
            load_weights_cast(self.i_d_nn, self.params['i_d_nn_config'], NBER_folder + "/pre_tech_post_damage/i_d_nn_checkpoint_pre_tech_post_damage", n_inputs)
            load_weights_cast(self.i_r_nn, self.params['i_r_nn_config'], NBER_folder + "/pre_tech_post_damage/i_I_nn_checkpoint_pre_tech_post_damage", n_inputs)

        ## Load pretrained weights
        if self.params['pretrained_path'] is not None:
            load_weights_cast(self.v_nn,   self.params['v_nn_config'],   self.params["pretrained_path"] + '/PostDamagePreTech/v_nn_checkpoint_PostDamagePreTech', n_inputs)
            load_weights_cast(self.i_g_nn, self.params['i_g_nn_config'], self.params["pretrained_path"] + '/PostDamagePreTech/i_g_nn_checkpoint_PostDamagePreTech', n_inputs)
            load_weights_cast(self.i_d_nn, self.params['i_d_nn_config'], self.params["pretrained_path"] + '/PostDamagePreTech/i_d_nn_checkpoint_PostDamagePreTech', n_inputs)
            load_weights_cast(self.i_r_nn, self.params['i_r_nn_config'], self.params["pretrained_path"] + '/PostDamagePreTech/i_r_nn_checkpoint_PostDamagePreTech', n_inputs)
