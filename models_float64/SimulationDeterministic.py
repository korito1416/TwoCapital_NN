...
def simulate_path_PreDamagePreTech(
    ...
    logK = tf.math.log(tf.constant(params["K0"], dtype=tf.float64))
    Z    = tf.constant(params["Z0"],    dtype=tf.float64)
    Y    = tf.constant(params["Y0"],    dtype=tf.float64)
    logR = tf.math.log(tf.constant(params["R0"], dtype=tf.float64))
    λ3   = tf.constant(0.0,   dtype=tf.float64)   # start at no damage
    logξ = tf.math.log(tf.constant(ξ, dtype=tf.float64))
    ...
    ta_xs = tf.TensorArray(dtype=tf.float64, size=nT)
    ta_ig = tf.TensorArray(dtype=tf.float64, size=nT)
    ta_id = tf.TensorArray(dtype=tf.float64, size=nT)
    ta_ir = tf.TensorArray(dtype=tf.float64, size=nT)

    # For distorted tech/damage weights per step: store scalars for the two tech branches
    ta_g_interm = tf.TensorArray(dtype=tf.float64, size=nT)
    ta_g_post = tf.TensorArray(dtype=tf.float64, size=nT)

    # damage weights are vectors of length L at each time 
    if L > 0:
        ta_g_dmg = tf.TensorArray(dtype=tf.float64, size=nT, element_shape=(L,))
    else:
        ta_g_dmg = None
    ...
    def _damage_weights(logK, Z, Y, logR, λ3, logξ_scalar) -> List[tf.Tensor]:
        ...
        for l in range(L):
            ...
            v_post = damage_continuation(
              tf.concat([logK, Z, tf.ones(tf.shape(Y), dtype=Y.dtype) * y_upper, logR,  λ3_l , logξ_scalar, logξ_scalar, logξ_scalar], 1)
            )
    ...
    if L > 0:
        ...
        g_dmg_arr_tf = tf.zeros((nT, 0), dtype=tf.float64)
    ...
    J_d_tf = r1  * (tf.exp(r2  / 2.0 * tf.pow(Y_path_tf - y_lower , 2.0)) - 1.0) * tf.cast(Y_path_tf > y_lower , tf.float64)
    ...
    try:
        if theta_ell_csv is not None and os.path.exists(theta_ell_csv):
            import pandas as pd
            theta_ell = (pd.read_csv(theta_ell_csv, header=None).to_numpy()[:,0]/1000).astype(np.float64)
            pi_c_o = np.ones_like(theta_ell, dtype=np.float64) / len(theta_ell)
            ϛ   = params['ϛ'] 
            θ_bar = params['θ_bar']
...
if __name__ == "__main__":
    ...
    # FLOAT64 sandbox: the trained checkpoints are float64 (models_float64 DAG).
    # set_floatx BEFORE any net is built so the regime models construct float64
    # nets and load_weights reads the float64 checkpoints without a dtype error.
