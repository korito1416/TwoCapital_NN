"""
Solve  pre-damage-post-technology  model.
...
class PreDamagePostTechModel:
    """Pre-damage & Post-technology HJB.
    """
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        X = tf.concat([logK, Z, Y, A_g_prime_prime *  tf.ones(tf.shape(Y), dtype=Y.dtype), logξ, logξ], 1)
        ...
        Jump_term = tf.zeros_like(dv_dZ) 
        
        for l in range(L): # L is the number of damage realizations
            λ3_l = λ3_values[l]
            
            v_PostDamagePostTech  = self.v_PostDamagePostTech_nn(  tf.concat([logK, Z, tf.ones(tf.shape(Y), dtype=Y.dtype) * y_upper,  λ3_l * tf.ones(tf.shape(Y), dtype=Y.dtype), A_g_prime_prime *tf.ones(tf.shape(Y), dtype=Y.dtype) ,logξ, logξ], 1) )
            g_l = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamagePostTech - v), -700.0, 350.0) )
    ...
    def train(self):
        ...
        if NBER_folder is not None:
            load_weights_cast(self.v_nn,   self.params['v_nn_config'],   NBER_folder + "/post_tech_pre_damage/v_nn_checkpoint_post_tech_pre_damage", n_inputs)
            load_weights_cast(self.i_g_nn, self.params['i_g_nn_config'], NBER_folder + "/post_tech_pre_damage/i_g_nn_checkpoint_post_tech_pre_damage", n_inputs)
            load_weights_cast(self.i_d_nn, self.params['i_d_nn_config'], NBER_folder + "/post_tech_pre_damage/i_d_nn_checkpoint_post_tech_pre_damage", n_inputs)



        ## Load pretrained weights
        if self.params['pretrained_path'] is not None:
            load_weights_cast(self.v_nn,   self.params['v_nn_config'],   self.params["pretrained_path"] + '/PreDamagePostTech/v_nn_checkpoint_PreDamagePostTech', n_inputs)
            load_weights_cast(self.i_g_nn, self.params['i_g_nn_config'], self.params["pretrained_path"] + '/PreDamagePostTech/i_g_nn_checkpoint_PreDamagePostTech', n_inputs)
            load_weights_cast(self.i_d_nn, self.params['i_d_nn_config'], self.params["pretrained_path"] + '/PreDamagePostTech/i_d_nn_checkpoint_PreDamagePostTech', n_inputs)
        ...
        self.v_nn.save_weights( self.params["export_folder"] + '/v_nn_checkpoint_PreDamagePostTech')
        self.i_g_nn.save_weights( self.params["export_folder"] + '/i_g_nn_checkpoint_PreDamagePostTech')
        self.i_d_nn.save_weights( self.params["export_folder"] + '/i_d_nn_checkpoint_PreDamagePostTech')
...
if __name__ == '__main__':
    ...
    test_model = PreDamagePostTechModel(params)
