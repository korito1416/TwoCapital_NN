...
class PreDamagePreTechModel:
    ...
    def __init__(self, params):
        ...
        if self.use_intermediate_tech_jump:
            self.v_PreDamageIntermTech_nn    = FeedForwardSubNet(self.params['v_nn_config'])
        ...
        self.v_PostDamagePreTech_nn    = FeedForwardSubNet(self.params['v_nn_config'])
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        v_PreDamagePostTech  = self.v_PreDamagePostTech_nn(tf.concat([logK, Z, Y, A_g_prime_prime *  tf.ones(tf.shape(Y), dtype=Y.dtype), logξ, logξ], 1) )

        g_l_prime_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PreDamagePostTech - v), -700.0, 350.0) )
        ...
        if self.use_intermediate_tech_jump:
            ...
            g_l_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PreDamageIntermTech - v), -700.0, 350.0) )
            Jump_term += J_g_prime * g_l_prime * (v_PreDamageIntermTech - v) + ξ * J_g_prime * (1- g_l_prime + g_l_prime * tf.math.log(g_l_prime) )
        
        ## Damage Jump
        J_d = r1 * ( tf.exp( r2 / 2 * tf.pow(Y - y_lower ,2) ) - 1  ) *  tf.cast(Y > y_lower, tf.float64 )

        for l in range(L): # L is the number of damage realizations
            λ3_l = λ3_values[l]
            
            v_PostDamagePreTech  = self.v_PostDamagePreTech_nn(   tf.concat([logK, Z, tf.ones(tf.shape(Y), dtype=Y.dtype) * y_upper, logR,  λ3_l * tf.ones(tf.shape(Y), dtype=Y.dtype), logξ, logξ, logξ], 1))
            g_l = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamagePreTech - v), -700.0, 350.0) )
            Jump_term +=  J_d / L * g_l * (v_PostDamagePreTech - v) + ξ * J_d / L * (1- g_l + g_l  * tf.math.log(g_l ) )
    ...
    def objective_fn(self, logK, Z, Y, logR, λ3, logξ,  compute_control = False, training = True):
        ...
        negative_consumption_boolean = tf.reshape( tf.cast( c < 1e-8, tf.float64 ),  [-1, 1])
        loss_c  = - c  * negative_consumption_boolean + epsilon
        
        negative_inside_log_i_g_boolean = tf.reshape( tf.cast( inside_log_i_g < 1e-8, tf.float64 ),  [-1, 1])
        loss_inside_log_i_g             = - inside_log_i_g  * negative_inside_log_i_g_boolean + epsilon
        
        negative_inside_log_i_d_boolean = tf.reshape( tf.cast( inside_log_i_d < 1e-8, tf.float64 ),  [-1, 1])
        ...
        if training:    
            ...
            if compute_control:
                ...
                loss_dv_dY = dv_dY  * tf.reshape( tf.cast(Y > self.params['y_upper'], tf.float64 ),  [-1, 1]) \
                    * tf.reshape( tf.cast( dv_dY > 0, tf.float64 ),  [-1, 1]) + 10e-8
                loss_dv_dlogR =  dv_dlogR  * tf.reshape( tf.cast( dv_dlogR < 0.0, tf.float64 ),  [-1, 1]) + 10e-8
            ...
            ## loss associated with dv/dY > 0
            loss_dv_dY = dv_dY * tf.reshape( tf.cast(Y > self.params['y_upper'], tf.float64 ),  [-1, 1]) \
                * tf.reshape( tf.cast( dv_dY > 0.0, tf.float64 ),  [-1, 1])  + 10e-8
    ...
    def train(self):
        ...
        # float64 sandbox: warm-start checkpoints are FLOAT32 (production runs);
        # load_weights_cast restores them into a float32 temp net and casts the
        # weights into this float64 net (plain load_weights would raise a dtype error).
        NBER_folder = legacy_nber_folder(required=self.params.get("pretrained_path") is None)
        if NBER_folder is not None:
            load_weights_cast(self.v_nn,   self.params['v_nn_config'],   NBER_folder + "/pre_tech_pre_damage/v_nn_checkpoint_pre_tech_pre_damage", n_inputs)
            load_weights_cast(self.i_g_nn, self.params['i_g_nn_config'], NBER_folder + "/pre_tech_pre_damage/i_g_nn_checkpoint_pre_tech_pre_damage", n_inputs)
            load_weights_cast(self.i_d_nn, self.params['i_d_nn_config'], NBER_folder + "/pre_tech_pre_damage/i_d_nn_checkpoint_pre_tech_pre_damage", n_inputs)
            load_weights_cast(self.i_r_nn, self.params['i_r_nn_config'], NBER_folder + "/pre_tech_pre_damage/i_I_nn_checkpoint_pre_tech_pre_damage", n_inputs)



        ## Load pretrained weights
        if self.params['pretrained_path'] is not None:
            load_weights_cast(self.v_nn,   self.params['v_nn_config'],   self.params["pretrained_path"] + '/PreDamagePreTech/v_nn_checkpoint_PreDamagePreTech', n_inputs)
            load_weights_cast(self.i_g_nn, self.params['i_g_nn_config'], self.params["pretrained_path"] + '/PreDamagePreTech/i_g_nn_checkpoint_PreDamagePreTech', n_inputs)
            load_weights_cast(self.i_d_nn, self.params['i_d_nn_config'], self.params["pretrained_path"] + '/PreDamagePreTech/i_d_nn_checkpoint_PreDamagePreTech', n_inputs)
            load_weights_cast(self.i_r_nn, self.params['i_r_nn_config'], self.params["pretrained_path"] + '/PreDamagePreTech/i_r_nn_checkpoint_PreDamagePreTech', n_inputs)
        ...
        # begin sgd iteration
        # begin sgd iteration
        # Bind `header` before the loop so the post-loop np.savetxt cannot raise
        # UnboundLocalError when an early nonfinite-break happens at step 0.
        ...
        self.i_g_nn.save_weights( self.params["export_folder"] + '/i_g_nn_checkpoint_PreDamagePreTech')
        self.i_d_nn.save_weights( self.params["export_folder"] + '/i_d_nn_checkpoint_PreDamagePreTech')
        self.i_r_nn.save_weights( self.params["export_folder"] + '/i_r_nn_checkpoint_PreDamagePreTech')
...
if __name__ == '__main__':
    ...
    # float64 sandbox: set the keras backend dtype BEFORE any network is built
    # (the model __init__ below constructs all FeedForwardSubNets).  This makes
    # every new layer + the state samples (via tf.keras.backend.floatx()) float64,
    # so exp(200)^2 in the damage-jump distortion no longer overflows.
