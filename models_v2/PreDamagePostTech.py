...
class PreDamagePostTechModel:
    ...
    def __init__(self, params):
        ...
        ### v2: the neighbor net reads a 7-column PostDamagePostTech input, so it
        ### must use PostDamagePostTech's input_bounds (NOT this regime's 6-col
        ### bounds). Build a config copy with the correct bounds/width.
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        for l in range(L): # L is the number of damage realizations
            ...
            expo_l = tf.clip_by_value(-1.0/ξ * (v_PostDamagePostTech - v), -50.0, 50.0)
            g_l = tf.exp(expo_l)
            Jump_term +=  J_d / L * g_l * (v_PostDamagePostTech - v) + ξ * J_d / L * (1.0 - g_l + g_l * expo_l)
        ...
        if training:
            ...
            if compute_control:
                ...
                return -tf.reduce_mean( (rhs - pv ) ) + \
                        ...
                        tf.sqrt(tf.reduce_mean(tf.square(FOC_d  ))) + penalty
    ...
    def train(self):
        ...
        early = EarlyStopper(
            nets=[self.v_nn, self.i_g_nn, self.i_d_nn],
            best_nets=[best_v_nn, best_i_g_nn, best_i_d_nn],
...
if __name__ == '__main__':
    ...
    _input_bounds = regime_input_bounds("PreDamagePostTech")
    ...
    for _cfg in (v_nn_config, i_g_nn_config, i_d_nn_config):
