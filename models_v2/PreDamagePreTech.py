...
class PreDamagePreTechModel:
    ...
    def objective_fn(self, logK, Z, Y, logR, λ3, logξ,  compute_control = False, training = True):
        ...
        if training:
            ## Always-on smooth feasibility penalty (replaces the data-dependent
            ## `if control_constraints>0` branch; review THEME 3.6). softplus(-x)
            ## activates smoothly as the argument approaches 0 from above.
            ## inside_log_i_g/inside_log_i_d are the RAW 1.0 + θ_* i_* returned by
            ## pde_rhs; c is the raw consumption.
            ...
            if compute_control:
                ## Optimizing all controls together (RAW FOC + feasibility penalty)
                return -tf.reduce_mean( (rhs - pv ) ) + \
                        tf.sqrt(tf.reduce_mean(tf.square(FOC_g  )))  + \
                        tf.sqrt(tf.reduce_mean(tf.square(FOC_d  ))) + \
                        tf.sqrt(tf.reduce_mean(tf.square(FOC_r  ))) + penalty
                ...
                loss = tf.sqrt(tf.reduce_mean(tf.square( (rhs - pv) / precond_w )))  \
                       + tf.sqrt(tf.reduce_mean(tf.square(FOC_g ))) \
                        ...
                        + tf.sqrt(tf.reduce_mean(tf.square(loss_dv_dY  )))  + tf.sqrt(tf.reduce_mean(tf.square(loss_dv_dlogR  )))  + penalty
    ...
    def train(self):
        ...
        if self.params['pretrained_path'] is not None:
            ...
            self.i_g_nn.load_weights( self.params["pretrained_path"]  + '/PreDamagePreTech/i_g_nn_checkpoint_PreDamagePreTech')
            self.i_d_nn.load_weights( self.params["pretrained_path"]  + '/PreDamagePreTech/i_d_nn_checkpoint_PreDamagePreTech')
            self.i_r_nn.load_weights( self.params["pretrained_path"]  + '/PreDamagePreTech/i_r_nn_checkpoint_PreDamagePreTech')
        ...
        # v2: patience-based early stopping with best-weights restore (incl. i_r_nn).
        early = EarlyStopper(
            nets=[self.v_nn, self.i_g_nn, self.i_d_nn, self.i_r_nn],
            best_nets=[best_v_nn, best_i_g_nn, best_i_d_nn, best_i_r_nn],
...
if __name__ == '__main__':
    ...
    # v2: attach fixed input-normalization bounds and seed to every net config
    # (including i_r_nn, which receives the same X as v/i_g/i_d -- spec C5).
    for _cfg in (v_nn_config, i_g_nn_config, i_d_nn_config, i_r_nn_config):
        _cfg["input_bounds"] = _input_bounds
        _cfg["seed"] = model_seed
