...
class PreDamagePostTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        ####   Replaces the (under-identified) i_g_nn / i_d_nn networks. Same algebra
        ####   as the PostDamagePostTech template (post-tech: no R&D control i_r):
        ####     FOC_d = -delta/c + q_d * Gamma_d * theta_d / (1+theta_d*i_d) = 0
        ####     FOC_g = -delta/c + q_g * Gamma_g * theta_g / (1+theta_g*i_g) = 0
        ####   with c = (A_d - i_d)(1-Z) + (A_g'' - i_g) Z solved algebraically.
        ###########
        eps_q = tf.constant(1e-3, tf.float32)
        # Admissibility clamp (mirrors the FD slope clamp): keep q_d>0, q_g>0 by
        # clipping the share-slope dv_dZ. q_d = dv_dlogK - Z*dv_dZ >= eps_q -> UPPER
        # bound on dv_dZ; q_g = dv_dlogK + (1-Z)*dv_dZ >= eps_q -> LOWER bound.
        ...
        # v3: c is ALREADY computed in closed form above (equals the resource
        # constraint (A_d - i_d)(1-Z) + (A_g'' - i_g)Z by construction). Do NOT recompute.
    ...
    def objective_fn(self, logK, Z, Y, logR, λ3, logξ,  compute_control = False, training = True):
        ...
        if training:
            ...
            ## v3: preconditioned HJB residual + dv/dY penalty + feasibility penalty.
            ## FOC_d/FOC_g are ~0 by construction (closed-form controls) -> dropped.
...
if __name__ == '__main__':
    ...
    # The control-net activation CLI positions ([1],[2]) are kept in the argv
    # contract for parity with the other regimes but are now unused.
