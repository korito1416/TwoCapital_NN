...
class PostDamagePreTechModel:
    ...
    def __init__(self, params):
        ...
        ### v2: neighbor nets read inputs with their OWN regime's column layout,
        ### so each gets that regime's input_bounds (NOT this regime's 8-col set).
        _post_cfg = dict(self.params['v_nn_config'])
        _post_cfg["input_bounds"] = regime_input_bounds("PostDamagePostTech")
        self.v_PostDamagePostTech_nn    = FeedForwardSubNet(_post_cfg)
        ...
        if self.use_intermediate_tech_jump:
            _interm_cfg = dict(self.params['v_nn_config'])
            _interm_cfg["input_bounds"] = regime_input_bounds("PostDamageIntermTech")
            self.v_PostDamageIntermTech_nn    = FeedForwardSubNet(_interm_cfg)
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        ####
        #### The three FOCs (from this file's FOC_d/FOC_g/FOC_r) at the solution:
        ...
        ####   with resource constraint  c = (A_d-i_d)(1-Z) + (A_g-i_g)Z - i_r.
        ####
        #### From FOC_d/FOC_g: i_j = Gamma_j*c*q_j/delta - 1/theta_j. Substituting
        #### into the constraint gives a CLOSED FORM for c GIVEN i_r:
        ####   c = delta*(N - i_r) / (delta + S),
        ####   N := Abar + (1-Z)/theta_d + Z/theta_g,   Abar := (1-Z)A_d + Z*A_g,
        ####   S := (1-Z)*Gamma_d*q_d + Z*Gamma_g*q_g.
        #### Substituting that c into FOC_r leaves a SCALAR equation in i_r alone:
        ####   F(i_r) = -(delta+S)/(delta*(N - i_r)) + psi1*B*i_r^(psi1-1) = 0.
        #### F is strictly decreasing on (0, N) (psi1=0.5<1, B>0): marginal benefit
        #### falls (i_r^{-0.5}) while marginal cost rises; unique root, i_r>0. We
        #### solve it with a short UNROLLED Newton (bracketed/clamped for safety).
        ###########
        ...
        # (0,N). If B>0 it has a unique interior root (i_r>0); if B<=0 (R&D has no
        # marginal value, e.g. dv_dlogR<=0 early in training) the marginal benefit
        # is nonpositive everywhere, so the optimum is the corner i_r -> floor (a
        # KKT boundary, where FOC_r<0; this is the i_r>=0 constraint binding).
        B_r = ψ0 * tf.exp(ψ1 * (logK - logR)) * dv_dlogR
        has_root = B_r > 0.0

        # feasible bracket for i_r keeping c>0: (i_r_min, N_r*(1-tiny))
        ...
        i_r = tf.stop_gradient(i_r)  # control treated as solved given v-derivatives
        ...
        # Given i_r, c and (i_d,i_g) are closed form.
        c = δ * (N_r - i_r) / (δ + S_r)
        ...
        expo_g_pp = tf.clip_by_value(-1.0/ξ * (v_PostDamagePostTech - v), -50.0, 50.0)
        g_l_prime_prime = tf.exp(expo_g_pp)
        Jump_term = J_g_prime_prime * g_l_prime_prime * (v_PostDamagePostTech - v) + ξ * J_g_prime_prime * (1.0 - g_l_prime_prime + g_l_prime_prime * expo_g_pp)

        if self.use_intermediate_tech_jump:
            v_PostDamageIntermTech  = self.v_PostDamageIntermTech_nn( tf.concat([logK, Z, Y, logR, λ3, logξ, logξ, logξ], 1))
            expo_g_p = tf.clip_by_value(-1.0/ξ * (v_PostDamageIntermTech - v), -50.0, 50.0)
            g_l_prime = tf.exp(expo_g_p)
            Jump_term += J_g_prime * g_l_prime * (v_PostDamageIntermTech - v) + ξ * J_g_prime * (1.0 - g_l_prime + g_l_prime * expo_g_p)
        ...
        # v3: at interior (root) points i_r solves FOC_r=0 exactly; at corner/KKT
        # points (i_r at floor, R&D shut off) the EQUALITY does not hold by design,
        # so mask FOC_r to the interior -> the reported FOC_r metric is ~0 and does
        # not corrupt validation_score with the (legitimate) corner residual.
...
if __name__ == '__main__':
    ...
    _input_bounds = regime_input_bounds("PostDamagePreTech")
