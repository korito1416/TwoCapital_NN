"""
Solve  pre-damage-pre-technology  model.
...
class PreDamagePreTechModel:
    ...
    def __init__(self, params):
        ...
        # v3: SEMI-ANALYTIC controls. i_d, i_g, AND the R&D rate i_r are computed
        # in CLOSED FORM from the value-net derivatives inside pde_rhs (i_d/i_g via
        # the two-capital FOCs; i_r via a vectorized Newton solve of FOC_r=0).
        # There are NO control networks; v_nn is the ONLY trained net.
        self.v_nn    = FeedForwardSubNet(self.params['v_nn_config'])

        ### Load weights from neighbor regimes.
        ### v2: each neighbor net reads inputs with its OWN regime's column layout,
        ### so each gets that regime's input_bounds (NOT this regime's 7-col set).
        _prepost_cfg = dict(self.params['v_nn_config'])
        _prepost_cfg["input_bounds"] = regime_input_bounds("PreDamagePostTech")
        self.v_PreDamagePostTech_nn    = FeedForwardSubNet(_prepost_cfg)
        ...
        if self.use_intermediate_tech_jump:
            _preinterm_cfg = dict(self.params['v_nn_config'])
            _preinterm_cfg["input_bounds"] = regime_input_bounds("PreDamageIntermTech")
            self.v_PreDamageIntermTech_nn    = FeedForwardSubNet(_preinterm_cfg)
            self.v_PreDamageIntermTech_nn.build((None, 7))
            self.v_PreDamageIntermTech_nn.load_weights(self.params["v_PreDamageIntermTech_nn_path"] )
        else:
            self.v_PreDamageIntermTech_nn = None

        _postpre_cfg = dict(self.params['v_nn_config'])
        _postpre_cfg["input_bounds"] = regime_input_bounds("PostDamagePreTech")
        self.v_PostDamagePreTech_nn    = FeedForwardSubNet(_postpre_cfg)
        self.v_PostDamagePreTech_nn.build((None, 8))
        self.v_PostDamagePreTech_nn.load_weights(self.params["v_PostDamagePreTech_nn_path"] )
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        #  tf.concat([logK, R, Y, log_I_g, log_xi, log_xi, log_xi], 1)
        # Controls defined in section 3.4
        # v3: i_d, i_g, i_r are computed in CLOSED FORM below (after the v-derivatives).
        v = self.v_nn(X)
        ...
        ###########
        #### CLOSED-FORM (semi-analytic) controls  [v3]   (pre-tech: HAS R&D i_r)
        ####
        #### Same algebra as PostDamagePreTech. FOCs (this file's FOC_d/FOC_g/FOC_r):
        ####   FOC_d: -delta/c + q_d * Gamma_d*theta_d/(1+theta_d*i_d) = 0
        ####   FOC_g: -delta/c + q_g * Gamma_g*theta_g/(1+theta_g*i_g) = 0
        ####   FOC_r: -delta/c + psi1 * B * i_r^(psi1-1) = 0,
        ####          B := psi0 * exp(psi1*(logK-logR)) * dv_dlogR,
        ####   constraint  c = (A_d-i_d)(1-Z) + (A_g-i_g)Z - i_r.
        #### => c = delta*(N - i_r)/(delta + S) with
        ####      N := Abar + (1-Z)/theta_d + Z/theta_g,  Abar := (1-Z)A_d + Z*A_g,
        ####      S := (1-Z)*Gamma_d*q_d + Z*Gamma_g*q_g.
        #### Substituting c into FOC_r gives a SCALAR equation in i_r:
        ####   F(i_r) = -(delta+S)/(delta*(N - i_r)) + psi1*B*i_r^(psi1-1) = 0,
        #### strictly decreasing on (0,N) => unique positive root (UNROLLED Newton).
        ###########
        ...
        Abar_r = (1.0 - Z) * A_d + Z * A_g            # pre-tech A_g in the constraint
        N_r = Abar_r + (1.0 - Z) / θ_d + Z / θ_g
        S_r = (1.0 - Z) * Γ_d * q_d + Z * Γ_g * q_g

        # Marginal-benefit coefficient B = psi0 * exp(psi1*(logK-logR)) * dv_dlogR,
        # using the RAW dv_dlogR so this exactly matches the FOC_r returned below.
        # FOC_r(i_r) = -delta/c(i_r) + psi1*B*i_r^(psi1-1); monotone DECREASING on
        # (0,N). B>0 => unique interior root (i_r>0); B<=0 (R&D has no marginal
        # value, e.g. dv_dlogR<=0 early in training) => corner optimum i_r->floor
        # (KKT boundary where FOC_r<0; the i_r>=0 constraint binds).
        B_r = ψ0 * tf.exp(ψ1 * (logK - logR)) * dv_dlogR
        has_root = B_r > 0.0
        ...
        def _focr(ir):
            c_of = δ * (N_r - ir) / (δ + S_r)
            return -δ / tf.maximum(c_of, 1e-8) + ψ1 * B_r * tf.pow(ir, ψ1 - 1.0)

        # VECTORIZED BISECTION (robust to the i_r^(psi1-1) stiffness that makes a
        # fixed-start Newton overshoot). 60 unrolled steps -> ~1e-18 bracket width.
        lo = tf.fill(tf.shape(N_r), i_r_min)
        hi = tf.ones_like(N_r) * i_r_hi
        for _ in range(60):
            mid = 0.5 * (lo + hi)
            go_right = _focr(mid) > 0.0   # FOC_r decreasing => root is to the right
            lo = tf.where(go_right, mid, lo)   # root in [mid, hi]
            hi = tf.where(go_right, hi, mid)   # else root in [lo, mid]
        i_r = tf.where(has_root, 0.5 * (lo + hi), i_r_min)
        i_r = tf.maximum(i_r, i_r_min)          # guarantee i_r > 0 strictly
        i_r = tf.stop_gradient(i_r)
        # Mask marking interior (root) points; used so the eval FOC_r metric is the
        # INTERIOR residual only (at corner/KKT points i_r is at its floor and the
        # FOC_r EQUALITY does not hold by design -- the i_r>=0 constraint binds).
        focr_interior_mask = tf.cast(has_root, tf.float32)
        ...
        # v3: c is ALREADY computed in closed form above (= the resource constraint
        # (A_d - i_d)(1-Z) + (A_g - i_g)Z - i_r by construction). Do NOT recompute.
        ...
        expo_g_pp = tf.clip_by_value(-1.0/ξ * (v_PreDamagePostTech - v), -50.0, 50.0)
        g_l_prime_prime = tf.exp(expo_g_pp)

        Jump_term = J_g_prime_prime * g_l_prime_prime * (v_PreDamagePostTech - v) + ξ * J_g_prime_prime * (1.0 - g_l_prime_prime + g_l_prime_prime * expo_g_pp)
        if self.use_intermediate_tech_jump:
            v_PreDamageIntermTech  = self.v_PreDamageIntermTech_nn(  tf.concat([logK, Z, Y, logR,   logξ, logξ, logξ], 1) )
            expo_g_p = tf.clip_by_value(-1.0/ξ * (v_PreDamageIntermTech - v), -50.0, 50.0)
            g_l_prime = tf.exp(expo_g_p)
            Jump_term += J_g_prime * g_l_prime * (v_PreDamageIntermTech - v) + ξ * J_g_prime * (1.0 - g_l_prime + g_l_prime * expo_g_p)
        ...
        for l in range(L): # L is the number of damage realizations
            ...
            v_PostDamagePreTech  = self.v_PostDamagePreTech_nn(   tf.concat([logK, Z, tf.ones(tf.shape(Y)) * y_upper, logR,  λ3_l * tf.ones(tf.shape(Y)), logξ, logξ, logξ], 1))
            expo_l = tf.clip_by_value(-1.0/ξ * (v_PostDamagePreTech - v), -50.0, 50.0)
            g_l = tf.exp(expo_l)
            Jump_term +=  J_d / L * g_l * (v_PostDamagePreTech - v) + ξ * J_d / L * (1.0 - g_l + g_l * expo_l)
        ...
        rhs = flow \
            ...
            + (-1.0) * v_logN_term \
            + v_logR_term * dv_dlogR + v_logRlogR_term * d2v_dlogR2 + 0.5 * ξ * tf.pow( h_r, 2) \
            + Jump_term
        ...
        # v3: mask FOC_r to interior (root) points (see closed-form block); at corner
        # KKT points i_r is at its floor and the equality does not hold by design.
        FOC_r = ( -marginal_util_c   + ψ0 * ψ1 * tf.exp( ψ1  *   ( tf.math.log(i_r) +logK -  logR) )* dv_dlogR  /i_r ) * focr_interior_mask
        ...
        precond_w = tf.abs(v_logK_term) + tf.abs(v_Z_term) / tf.maximum(Z * (1.0 - Z), 1e-3) \
            ...
            + tf.abs(v_y_term) \
            + tf.abs(v_logR_term) + v_logRlogR_term + eps
        precond_w = tf.stop_gradient(precond_w)

        return rhs, pv, dv_dY, c, 1.0 + θ_g * i_g , 1.0 + θ_d * i_d,  FOC_d, FOC_g , FOC_r , dv_dlogR , precond_w
    ...
    def objective_fn(self, logK, Z, Y, logR, λ3, logξ,  compute_control = False, training = True):
        ...
        rhs, pv, dv_dY, c, inside_log_i_g , inside_log_i_d ,  FOC_d, FOC_g,  FOC_r, dv_dlogR , precond_w = self.pde_rhs(logK, Z, Y, logR, λ3, logξ)
        ...
        if training:
            ...
            ## loss associated with dv/dY > 0 and dv/dlogR > 0 (monotonicity)
            ...
            ## v3: preconditioned HJB residual + dv/dY & dv/dlogR penalties + feasibility.
            ## FOC_d/FOC_g/FOC_r are ~0 by construction (closed-form controls) -> dropped.
            loss = tf.sqrt(tf.reduce_mean(tf.square( (rhs - pv) / precond_w )))  \
                    + tf.sqrt(tf.reduce_mean(tf.square(loss_dv_dY  ))) \
                    + tf.sqrt(tf.reduce_mean(tf.square(loss_dv_dlogR  ))) + penalty
            ...
            ## Keep the 5-tuple (loss_v, FOC_d, FOC_g, FOC_r, loss_dv_dY) for
            ## validation_score / large_sample_validation / lbfgs. FOCs are ~0.
    ...
    def grad(self, logK, Z, Y, logR, λ3, logξ, compute_control = False, training = True):
        ...
        ## v3: ONLY the value net is trainable (controls are closed-form, incl. the
        ## Newton-solved i_r). compute_control is ignored (kept for API parity).
    ...
    def train(self):
        ...
        # v3: ONLY the value net exists (controls are closed-form, incl. i_r).
        ...
        # v2: skip the NBER warm-start for a true from-scratch base (v2 checkpoints
        # are not NBER-load-compatible). FROM_SCRATCH=1 -> train_from_scratch.
        ...
        if self.params['pretrained_path'] is not None:
            self.v_nn.load_weights( self.params["pretrained_path"]  + '/PreDamagePreTech/v_nn_checkpoint_PreDamagePreTech')
        ...
        ## v3: value net only; controls are closed-form.
        self.v_nn.save_weights( self.params["export_folder"] + '/v_nn_checkpoint_PreDamagePreTech')
...
if __name__ == '__main__':
    ...
    _input_bounds = regime_input_bounds("PreDamagePreTech")


    # v3: ONLY the value net is built (controls are semi-analytic / closed-form,
    # including the Newton-solved i_r). The control-net activation CLI positions
    # ([1],[2],[3]) are kept in the argv contract for parity but are now unused.
    ...
    test_model = PreDamagePreTechModel(params)
