...
class FeedForwardSubNet(tf.keras.Model):
    def __init__(self, config):
        ...
        kernel_initializer = initializer, 
        activation=config['final_activation'], use_bias = False, name = config["nn_name"] + ".output" ))
...
class model:
    ...
    def __init__(self, params):
        ...
        if "pre_" in self.params["model_type"] and "post_" in self.params["model_type"]:
            ...
            self.v_post_tech_post_damage_nn.build( (self.params["batch_size"], 5) )  ## need to build network; pre_tech has four state variables so we don't need to do anything (remember gamma_3 is a pseudo state variable)
            ## 4 inputs here: logK, R, Y, gamma_3
        ...
        if "pre_tech" in self.params["model_type"] and "pre_damage" in self.params["model_type"]:
            ...
            ## 5 inputs here: logK, R, Y, gamma_3, and log_I_g
            ...
            self.v_post_tech_pre_damage_nn.build( (self.params["batch_size"], 4) ) 
            ## 3 inputs here: logK, R, Y
    ...
    def sample(self):
        ...
        if self.params["n_dims"] == 4:
            ...
            return logK, R, Y, gamma_3, log_xi, log_I_g

        else:
            return logK, R, Y, gamma_3, log_xi 

    @tf.function
    def pde_rhs(self, logK, R, Y, gamma_3, log_xi, log_I_g = None, training = True):
        ...
        if "pre_tech" in self.params["model_type"] and "pre_damage" in self.params["model_type"]:
            ...
            v_diff                        = v_post_tech_pre_damage - v
        ...
        if "post_tech" in self.params["model_type"] and "pre_damage" in self.params["model_type"]:
                ...
                for k in range(self.params["gamma_3_length"]):
                    X_post_tech_post_damage                   = tf.concat([logK, R, tf.ones(tf.shape(Y)) * self.params["y_bar"], tf.ones(tf.shape(Y)) * self.params["gamma_3_list"][k], log_xi], 1)
        ...
        if self.params["n_dims"] == 4:
            ...
            if "pre_tech" in self.params["model_type"] and "pre_damage" in self.params["model_type"]:
                rhs = rhs + tf.exp(log_I_g) / self.params["varrho"] * g * (v_post_tech_pre_damage - v) +  \
