...
def simulate_path_PreDamagePreTech(
    ...
    y_path_tf =  A_d * (1.0 - Z_path_tf) * K_path_tf  + A_g * Z_path_tf * K_path_tf
    c_path_tf = (A_d - id_path_tf) * (1.0 - Z_path_tf) + (A_g - ig_path_tf) * Z_path_tf - ir_path_tf
    RD_path_tf = ir_path_tf/( A_d * (1.0 - Z_path_tf) + A_g * Z_path_tf)
    ...
    export_folder = params.get("export_folder", ".") + "/SimulationOutputs" +f"_ξ_{ξ:.3f}"
    ...
    data = {
        ...
        "I_g": I_g, "I_d": I_d, "I_r": I_r, "E": E_path, "c": c_path, "RD": RD_path,
    ...
    try:
        ...
        # baseline probabilities placed at the requested positions:
        # interm (at A_g_prime) has weight 1-pi, post (at A_g_prime_prime) has weight pi
