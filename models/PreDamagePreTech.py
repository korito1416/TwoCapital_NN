...
class PreDamagePreTechModel:
    ...
    def __init__(self, params):
        ...
        self.v_PreDamageIntermTech_nn.build( (self.params["batch_size"], 7) )
        ...
        self.v_PostDamagePreTech_nn.build( (self.params["batch_size"], 8) )
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        d2v_dlogR2                = tf.reshape(tf.gradients(dv_dlogR, logR, unconnected_gradients='zero')[0], [self.params['batch_size'], 1])
    ...
    def train(self):
        ...
        if self.params['pretrained_path'] is not None:
            self.v_nn.load_weights( self.params["pretrained_path"]  + '/v_nn_checkpoint_PreDamagePreTech')
            self.i_g_nn.load_weights( self.params["pretrained_path"]  + '/i_g_nn_checkpoint_PreDamagePreTech')
            self.i_d_nn.load_weights( self.params["pretrained_path"]  + '/i_d_nn_checkpoint_PreDamagePreTech')
            self.i_r_nn.load_weights( self.params["pretrained_path"]  + '/i_r_nn_checkpoint_PreDamagePreTech')
