...
class PreDamageIntermTechModel:
    ...
    def __init__(self, params):
        ...
        self.v_PostDamageIntermTech_nn.build( (self.params["batch_size"], 6) )
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        for l in range(L): # L is the number of damage realizations
            ...
            v_PostDamageIntermTech  = self.v_PostDamageIntermTech_nn( tf.concat([logK, Z, tf.ones(tf.shape(Y)) * y_upper, logR,  λ3_l * tf.ones(tf.shape(Y)), logξ], 1) )
    ...
    def train(self):
        ...
        if self.params['pretrained_path'] is not None:
            self.v_nn.load_weights( self.params["pretrained_path"]  + '/v_nn_checkpoint_PreDamageIntermTech')
            self.i_g_nn.load_weights( self.params["pretrained_path"]  + '/i_g_nn_checkpoint_PreDamageIntermTech')
            self.i_d_nn.load_weights( self.params["pretrained_path"]  + '/i_d_nn_checkpoint_PreDamageIntermTech')
            self.i_r_nn.load_weights( self.params["pretrained_path"]  + '/i_r_nn_checkpoint_PreDamageIntermTech')
