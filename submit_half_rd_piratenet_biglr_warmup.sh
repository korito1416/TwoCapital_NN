...
PREFIX="/project/lhansen/Cap_damage/TwoStageTechJump_FOCIr_orignal/output_piratenet_biglr_warmup"
...
submit_variant() {
    ...
    foldername="TwoStageTech_RDIntensityScale_0p5_piratenet_bigLR_${SCHEDULE_TYPE}_${learning_rates}_${BATCH_SIZE}_neurons_${num_neurons}_#HiddenLayer_${num_hidden_layers}_num_iterations${NUM_ITERATIONS2}"
    job_name="${PREFIX}/${foldername}"
    mkdir -p "$job_name" "./job-outs/${foldername}"

    cat > "${job_name}/run_manifest.txt" <<EOF
Run: half technology jump intensity PirateNet big-LR warmup-cosine sweep
Created: $(date)
Tech jump intensity scale: ${TECH_JUMP_INTENSITY_SCALE}
Model directory: ${MODEL_DIR}
Pretrained source: ${PRETRAINED_FOLDER}
Learning rates for all six stages: ${learning_rates}
...
Note: Post-tech regimes only use the first two optimizer LRs internally; active regimes use all four.
EOF
    ...
    printf 'Submitted PirateNet big-LR warmup variant %s\n' "$foldername"
...
for learning_rates in "${LR_GRID[@]}"; do
    submit_variant "64" "6" "$learning_rates"
    submit_variant "128" "6" "$learning_rates"
done
