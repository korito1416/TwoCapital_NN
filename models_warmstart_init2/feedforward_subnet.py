import math
import os
_INIT_SEED = int(os.environ.get("MODEL_INIT_SEED", "0"))
...
class FeedForwardSubNet(tf.keras.Model):
    def __init__(self, config):
        ...
        if config['activation'] is not None and "relu" in config['activation']:
            initializer = tf.keras.initializers.HeNormal(seed=_INIT_SEED)
        else:
            initializer = tf.keras.initializers.GlorotUniform(seed=_INIT_SEED)
...
def sample_state_columns(params, batch_size=None):
    ...
    return (
        ...
        stratified_uniform(params.get("logxi_lo_var", params.get("logξ_min", -3.0)), params.get("logξ_max", 5.0), n),
    )
