...
class PreDamagePreTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        g_l_prime_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PreDamagePostTech - v), -60.0, 60.0))
        ...
        if self.use_intermediate_tech_jump:
            ...
            g_l_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PreDamageIntermTech - v), -60.0, 60.0))
        ...
        for l in range(L): # L is the number of damage realizations
            ...
            g_l = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamagePreTech - v), -60.0, 60.0))
    ...
    @tf.function
    def train_step_value_only(self):
        logK, Z, Y, logR, λ3, logξ = self.sample()
    ...
    def train(self):
        ...
        NBER_folder = None if os.environ.get("WARMSTART_MODE", "").upper() == "COLD" else legacy_nber_folder(required=self.params.get("pretrained_path") is None)
        ...
        # begin sgd iteration
        # begin sgd iteration
        if self.params.get("xi_curriculum", False):
            self._logxi_lo = tf.Variable(float(self.params.get("logxi_curriculum_start", 4.0)),
                                         dtype=tf.float32, trainable=False)
            self.params["logxi_lo_var"] = self._logxi_lo
        for step in range(self.params["num_iterations"]):
            if self.params.get("xi_curriculum", False):
                _t = int(self.params["num_iterations"]); _w = max(1, int(_t * float(self.params.get("xi_curriculum_frac", 0.3))))
                _s0 = float(self.params.get("logxi_curriculum_start", 4.0)); _tg = float(self.params.get("logξ_min", -3.0))
                self._logxi_lo.assign(_tg if step >= _w else _tg + (_s0 - _tg) * (1.0 - step / _w))
            if step < int(self.params.get("control_freeze_iters", 0)):
                loss_v_train, loss_c_train = self.train_step_value_only()
            else:
                loss_v_train, loss_c_train = self.train_step()
            _ck_every = int(os.environ.get("CHECKPOINT_EXPORT_EVERY", "0"))
            if _ck_every > 0 and step > 0 and step % _ck_every == 0:
                _ck = self.params["export_folder"] + f"/ckpt_step{step}"
                if not os.path.isdir(_ck):
                    os.makedirs(_ck, exist_ok=True)
                    self.v_nn.save_weights(_ck + "/v_nn_checkpoint_PreDamagePreTech")
                    self.i_g_nn.save_weights(_ck + "/i_g_nn_checkpoint_PreDamagePreTech")
                    self.i_d_nn.save_weights(_ck + "/i_d_nn_checkpoint_PreDamagePreTech")
                    self.i_r_nn.save_weights(_ck + "/i_r_nn_checkpoint_PreDamagePreTech")
...
if __name__ == '__main__':
    ...
    xi_curriculum                    = os.environ.get("XI_CURRICULUM", "0").lower() in {"1", "true", "yes", "on"}
    logxi_curriculum_start           = float(os.environ.get("LOGXI_CURRICULUM_START", "4.0"))
    xi_curriculum_frac               = float(os.environ.get("XI_CURRICULUM_FRAC", "0.3"))
    control_freeze_iters             = int(os.environ.get("CONTROL_FREEZE_ITERS", "0"))
    ...
    "xi_curriculum": xi_curriculum, "logxi_curriculum_start": logxi_curriculum_start,
    "xi_curriculum_frac": xi_curriculum_frac, "control_freeze_iters": control_freeze_iters,
