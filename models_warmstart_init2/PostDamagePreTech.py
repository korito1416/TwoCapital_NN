...
class PostDamagePreTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        g_l_prime_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamagePostTech - v), -60.0, 60.0))
        ...
        if self.use_intermediate_tech_jump:
            ...
            g_l_prime = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamageIntermTech - v), -60.0, 60.0))
        ...
        for step in range(self.params["num_iterations"]):
            ...
            if _ck_every > 0 and step > 0 and step % _ck_every == 0:
                ...
                if not os.path.isdir(_ck):
                    ...
                    self.v_nn.save_weights(_ck + "/v_nn_checkpoint_PostDamagePreTech")
                    self.i_g_nn.save_weights(_ck + "/i_g_nn_checkpoint_PostDamagePreTech")
                    self.i_d_nn.save_weights(_ck + "/i_d_nn_checkpoint_PostDamagePreTech")
                    self.i_r_nn.save_weights(_ck + "/i_r_nn_checkpoint_PostDamagePreTech")
