...
class PreDamagePostTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        for l in range(L): # L is the number of damage realizations
            ...
            v_PostDamagePostTech  = self.v_PostDamagePostTech_nn(  tf.concat([logK, Z, tf.ones(tf.shape(Y)) * y_upper,  λ3_l * tf.ones(tf.shape(Y)), A_g_prime_prime *tf.ones(tf.shape(Y)) ,logξ, logξ], 1) )
            g_l = tf.exp(tf.clip_by_value(-1/ξ * (v_PostDamagePostTech - v), -60.0, 60.0))
    ...
    def train(self):
        ...
        NBER_folder = None if (os.environ.get("ABLATION_SKIP_NBER", "0") == "1" or os.environ.get("WARMSTART_MODE", "").upper() == "COLD") else legacy_nber_folder(required=self.params.get("pretrained_path") is None)
        if NBER_folder is not None:
            ...
            self.i_d_nn.load_weights( NBER_folder + "/post_tech_pre_damage/i_d_nn_checkpoint_post_tech_pre_damage" )
        ...
        if self.params['pretrained_path'] is not None:
            ...
            self.i_g_nn.load_weights( self.params["pretrained_path"]  + '/PreDamagePostTech/i_g_nn_checkpoint_PreDamagePostTech')
            self.i_d_nn.load_weights( self.params["pretrained_path"]  + '/PreDamagePostTech/i_d_nn_checkpoint_PreDamagePostTech')
        ...
        for step in range(self.params["num_iterations"]):
            ...
            if _ck_every > 0 and step > 0 and step % _ck_every == 0:
                ...
                if not os.path.isdir(_ck):
                    ...
                    self.v_nn.save_weights(_ck + "/v_nn_checkpoint_PreDamagePostTech")
                    self.i_g_nn.save_weights(_ck + "/i_g_nn_checkpoint_PreDamagePostTech")
                    self.i_d_nn.save_weights(_ck + "/i_d_nn_checkpoint_PreDamagePostTech")
