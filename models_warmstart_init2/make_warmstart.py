...
def main():
    ...
    ap.add_argument("--mode", required=True, choices=["perturb", "analytic", "anchor", "constctrl", "anchor2"])
    ...
    for reg in REGS:
        ...
        if a.mode == "analytic":
        ...
        elif a.mode == "constctrl":
            # P1/valuefirst prep: cold value net (MODEL_INIT_SEED-driven), controls at
            # FOC-consistent constant rates (repairs the flat-value control collapse).
            save(fresh("v", DIMS[reg]), a.out, reg, "v")
            for nm in NETS[reg][1:]:
                net = fit(fresh(nm, DIMS[reg]), X, np.full_like(lk, REF_RATES[nm]),
                          steps=2500, tag=f"{reg}/{nm}")
                save(net, a.out, reg, nm)
            print(f"  constctrl init written for {reg}")

        elif a.mode == "anchor2":
            # P3: anchor + FOC-consistent constant controls. Terminal state's nets are the
            # arm's own trained post-post (left untouched); non-terminal v regressed onto it,
            # controls re-fit to constants.
            ...
            for nm in NETS[reg][1:]:
                net = fit(fresh(nm, DIMS[reg]), X, np.full_like(lk, REF_RATES[nm]),
                          steps=2500, tag=f"{reg}/{nm}")
                save(net, a.out, reg, nm)
            print(f"  anchor2 init written for {reg}")
