...
class PostDamagePostTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        # --- redesign: input layout (legacy == production; unified drops logξ padding + dead A_g'') ---
        if config.USE_UNIFIED:
            θ_unc = uncertainty.logxi_to_theta(logξ) if not config.USE_THETA else logξ
            X = state_layout.build_input("PostDamagePostTech", logK, Z, Y,
                                         logR=None, lam3=λ3,
        ...
        if config.ANCHOR == "recenter" or config.DETREND:
            ...
            # ξ AT THE ANCHOR STATE.  With config.ANCHOR_XI == "batch" the anchor inherits the
            # sample's own ξ, which forces v(x0, ξ) = v0 for EVERY ξ and therefore ANNIHILATES the
            # ξ-dependence of the value at x0 (measured exactly 0.00000 on 2026-07-26).  That
            # destroys the welfare cost of robustness V(X0,ξ) − V(X0,neutral) — the object we
            # report.  Default "neutral" pins the anchor at a FIXED neutral ξ, so exactly ONE
            # scalar (the level) is fixed and the whole ξ-profile stays free.
            if config.ANCHOR_XI == "batch":
                xi_anchor_col = None            # inherit from X (legacy behaviour, kept for A/B)
            else:
                xi_anchor_col = tf.ones_like(logK) * (
                    0.0 if config.USE_THETA else config.ANCHOR_LOGXI_NEUTRAL)
            if config.USE_UNIFIED:
                # unified slots: [logK, Z, Y, u, lam3, xi]; u = 0 post-tech (R&D off)
                xi_col = X[:, 5:6] if xi_anchor_col is None else uncertainty.network_input(xi_anchor_col) \
                    if config.USE_THETA else xi_anchor_col
                x0 = tf.concat([logK0, Z0, Y0, tf.zeros_like(logK), l30, xi_col], 1)
            else:
                # legacy slots: [logK, Z, Y, lam3, A_g'', logxi, logxi]
                if xi_anchor_col is None:
                    x0 = tf.concat([logK0, Z0, Y0, l30, X[:, 4:]], 1)
                else:
                    x0 = tf.concat([logK0, Z0, Y0, l30, X[:, 4:5],
                                    xi_anchor_col, xi_anchor_col], 1)
        ...
        # ξ(h_d²+h_g²+h_y²)/2 in the finite θ-form
        drift_penalty = 0.5 * θ_unc * (tf.pow(s_d, 2) + tf.pow(s_g, 2) + tf.pow(s_y, 2))
        ...
        _mb_d = Γ_d * θ_d / ( inside_log_i_d ) * (dv_dlogK - Z * dv_dZ)
        _mb_g = Γ_g * θ_g / ( inside_log_i_g )   * (dv_dlogK +  (1.0 - Z) * dv_dZ)
        FOC_d = -marginal_util_c + _mb_d
        FOC_g = -marginal_util_c + _mb_g
        # envelope of each FOC's OWN two terms (marginal utility vs marginal benefit): same
        # unit-covariant principle as the HJB envelope, so no term is hand-picked.
        foc_env = hjb_scaling.term_envelope((marginal_util_c, 0.5 * (_mb_d + _mb_g)))
  
        # `scale_parts` carries what hjb_scaling needs to build the per-state natural scale.
        scale_parts = (logK, v, c, dv_dY * v_y_term)
        # the ACTUAL assembled terms of this HJB, for the principled (unit-covariant) envelope scale
        hjb_terms = (flow, -pv,
                     v_logK_term * dv_dlogK, v_logKlogK_term * d2v_dlogK2,
                     v_Z_term * dv_dZ, v_ZZ_term * d2v_dZ2, v_logK_Z_term * d2v_dlogKdZ,
                     dv_dY * v_y_term, v_yy_term * d2v_dY2, drift_penalty,
                     (-1.0) * v_logN_term)
        env_scale = hjb_scaling.term_envelope(hjb_terms)
        # normalisers for the ORDER-HOMOGENEOUS xi-sensitivity losses:
        #   s_sq  -> the theta-explicit source of the HJB, (theta/2)||s||^2
        #   marginal_util_c -> the FOC's own scale
        s_sq = tf.pow(s_d, 2) + tf.pow(s_g, 2) + tf.pow(s_y, 2)
        sens_parts = (s_sq, marginal_util_c, θ_unc, env_scale, foc_env)
        return rhs, pv, dv_dY, c, 1.0 + θ_g * i_g , 1.0 + θ_d * i_d,  FOC_d, FOC_g, scale_parts, sens_parts
    ...
    def objective_fn(self, logK, Z, Y, logR, λ3, logξ,  compute_control = False, training = True):
        ...
        _use_sens = (config.SENS_WEIGHT > 0.0) or (config.FOCXI_WEIGHT > 0.0)
        if _use_sens:
            # d/d ln(theta) = -d/d(logxi)  (theta = exp(-logxi)), so differentiating w.r.t. logxi
            # and negating gives the log-theta derivative used by the order-homogeneous losses.
            with tf.GradientTape(persistent=True) as _xt:
                _xt.watch(logξ)
                rhs, pv, dv_dY, c, inside_log_i_g , inside_log_i_d ,  FOC_d, FOC_g, scale_parts, sens_parts = self.pde_rhs(logK, Z, Y, logR, λ3, logξ)
                _R = rhs - pv
            dR_dlnth   = -_xt.gradient(_R,    logξ)
            dFOCd_dlnth = -_xt.gradient(FOC_d, logξ)
            dFOCg_dlnth = -_xt.gradient(FOC_g, logξ)
            del _xt
        else:
            rhs, pv, dv_dY, c, inside_log_i_g , inside_log_i_d ,  FOC_d, FOC_g, scale_parts, sens_parts = self.pde_rhs(logK, Z, Y, logR, λ3, logξ)
            dR_dlnth = dFOCd_dlnth = dFOCg_dlnth = None
        ...
        if training:    
            ...
            if compute_control:
                ...
                _ctrl = -tf.reduce_mean( (rhs - pv ) ) + \
                        tf.sqrt(tf.reduce_mean(tf.square(FOC_g  )))  + \
                        tf.sqrt(tf.reduce_mean(tf.square(FOC_d  )))
                if _use_sens and config.FOCXI_WEIGHT > 0.0:
                    _s_sq, _mu, _th = sens_parts
                    _mud = tf.maximum(tf.abs(_mu), config.SENS_FLOOR)
                    _ctrl += config.FOCXI_WEIGHT * (
                        tf.sqrt(tf.reduce_mean(tf.square(dFOCg_dlnth / _mud)))
                        + tf.sqrt(tf.reduce_mean(tf.square(dFOCd_dlnth / _mud))))
                return _ctrl
                ...
                if config.LOSS_NONDIM:
                    # PRECONDITION: every term becomes a dimensionless RELATIVE error, so the sum is
                    # a well-posed objective instead of an accidental unit-weighted mixture.
                    # PRINCIPLED scales: HJB by the envelope of its own assembled terms
                    # (unit-covariant); FOCs and monotonicity by the marginal utility.
                    _env = sens_parts[3]
                    _sfoc = sens_parts[4]      # FOC envelope, same principle as the HJB envelope
                    _rms = lambda a: tf.sqrt(tf.reduce_mean(tf.square(a)))
                    loss = (_rms((rhs - pv) / _env) + _rms(FOC_g / _sfoc)
                            + _rms(FOC_d / _sfoc) + _rms(loss_dv_dY / _sfoc))
                else:
                    loss = hjb_term  \
                           + tf.sqrt(tf.reduce_mean(tf.square(FOC_g ))) \
                            + tf.sqrt(tf.reduce_mean(tf.square(FOC_d  ))) \
                ...
                if _use_sens:
                    _s_sq, _mu, _th = sens_parts
                    # ORDER-HOMOGENEOUS: each derivative is divided by the scale that GENERATES it,
                    # so both terms are O(1) and cannot be swamped by the ~1e-3 HJB/FOC terms.
                    if config.SENS_WEIGHT > 0.0:
                        _den = tf.maximum(0.5 * _th * _s_sq, config.SENS_FLOOR)
                        loss += config.SENS_WEIGHT * tf.sqrt(tf.reduce_mean(
                            tf.square(dR_dlnth / _den)))
                    if config.FOCXI_WEIGHT > 0.0:
                        _mud = tf.maximum(tf.abs(_mu), config.SENS_FLOOR)
                        loss += config.FOCXI_WEIGHT * (
                            tf.sqrt(tf.reduce_mean(tf.square(dFOCg_dlnth / _mud)))
                            + tf.sqrt(tf.reduce_mean(tf.square(dFOCd_dlnth / _mud))))
    ...
    def train(self):
        ...
        n_inputs = state_layout.WIDTH if config.USE_UNIFIED else 7
