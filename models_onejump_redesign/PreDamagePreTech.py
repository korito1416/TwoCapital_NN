...
class PreDamagePreTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        # --- redesign: input layout (legacy == production; unified drops logξ padding) -------
        if config.USE_UNIFIED:
            θ_unc = uncertainty.logxi_to_theta(logξ) if not config.USE_THETA else logξ
            X = state_layout.build_input("PreDamagePreTech", logK, Z, Y,
                                         logR=logR, lam3=None,
        ...
        if config.ANCHOR == "recenter" or config.DETREND:
            ...
            # pre-damage regimes live at the calibrated initial condition Y0 = 1.2.
            Y0    = tf.ones_like(Y)    * 1.2
            # logR is a TRUE state here (dv_dlogR is taken), so the anchor must hold it FIXED --
            # reusing the sampled logR would corrupt dv_dlogR.  R0 = 11.2 is the calibration.
            logR0 = tf.ones_like(logR) * 2.4159       # log(11.2)
            ...
            if config.USE_UNIFIED:
                # unified slots: [logK, Z, Y, u, lam3, xi]; u = logR - logK, lam3 = 0 pre-damage
                ...
                x0 = tf.concat([logK0, Z0, Y0, logR0 - logK0, tf.zeros_like(logK), _xa], 1)
            else:
                # legacy slots: [logK, Z, Y, logR, logxi, logxi, logxi] -> reuse cols 4: verbatim
                x0 = tf.concat([logK0, Z0, Y0, logR0, _xa, _xa, _xa], 1)
        ...
        # dV/dY = dv_dY - d(log N)/dY and d(log N)/dY = λ1 + λ2 * Y   (pre-damage)
        s_y = ( dv_dY  - (λ1  + λ2 * Y   )   ) *    η *  A_d * (1-Z) * K     *  ϛ
        ...
        if self.use_intermediate_tech_jump:
            ...
            g_l_prime = tf.exp(-1/ξ * (v_PreDamageIntermTech - v) )
        ...
        for l in range(L): # L is the number of damage realizations
            ...
            g_l = tf.exp(-1/ξ * (v_PostDamagePreTech - v) )
