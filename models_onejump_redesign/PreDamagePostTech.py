...
class PreDamagePostTechModel:
    ...
    def pde_rhs(self, logK, Z, Y, logR, λ3, logξ):
        ...
        if config.USE_UNIFIED:
            ...
            X = state_layout.build_input("PreDamagePostTech", logK, Z, Y,
                                         logR=None, lam3=None,
        ...
        if config.ANCHOR == "recenter" or config.DETREND:
            ...
            # pre-damage regimes are anchored at the calibrated initial condition Y0 = 1.2
            # (λ3 is NOT an input in this regime, so there is no λ3 slot to anchor).
            Y0    = tf.ones_like(Y)    * 1.2
            # xi AT THE ANCHOR STATE: pinned at a FIXED neutral xi (defined BEFORE the branch so both
            # the unified and legacy layouts see it; inheriting the batch xi would annihilate the
            # xi-response at x0 and destroy the welfare-cost-of-robustness object).
            ...
            if config.USE_UNIFIED:
                ...
                # lam3 = 0 pre-damage (curvature not yet revealed)
                ...
                x0 = tf.concat([logK0, Z0, Y0, tf.zeros_like(logK), tf.zeros_like(logK), _xa], 1)
            else:
                # legacy slots: [logK, Z, Y, A_g'', logxi, logxi] -> reuse cols 3: verbatim
                x0 = tf.concat([logK0, Z0, Y0, X[:, 3:4], _xa, _xa], 1)
        ...
        return rhs, pv, dv_dY, c, 1.0 + θ_g * i_g , 1.0 + θ_d * i_d,  FOC_d, FOC_g, scale_parts
    ...
    def objective_fn(self, logK, Z, Y, logR, λ3, logξ,  compute_control = False, training = True):
        ...
        rhs, pv, dv_dY, c, inside_log_i_g , inside_log_i_d ,  FOC_d, FOC_g, scale_parts = self.pde_rhs(logK, Z, Y, logR, λ3, logξ)
    ...
    def train(self):
        ...
        n_inputs = state_layout.WIDTH if config.USE_UNIFIED else 6
