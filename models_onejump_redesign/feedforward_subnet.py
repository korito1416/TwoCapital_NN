...
def sample_uncertainty_column(params, n):
    """Sample the uncertainty pseudo-state column.

    LEGACY (production): logξ ~ U[logξ_min, logξ_max].
    THETA  (Lars):       θ = 1/ξ, returned IN PLACE of logξ (the regime code reads the switch).

    Sampling θ ~ U[0, θ_max] puts ~80% of the batch at ξ<0.1 (vs 8.6% under uniform logξ), which
    fixes the documented deep-ξ underweight -- but it collapses the near-neutral region from 33.8%
    to 0.2%, and the neutral solution anchors every ξ-comparison we report.  REDESIGN_XI_SAMPLING
    therefore defaults to a MIXTURE: half the batch uniform in θ (deep coverage, includes θ=0
    exactly), half uniform in logξ mapped to θ (neutral coverage).
    """
    import config as _cfg

    if not _cfg.USE_THETA:
        return stratified_uniform(params.get("logξ_min", -3.0), params.get("logξ_max", 5.0), n)

    theta_max = params.get("θ_max", _cfg.THETA_MAX)
    mode = str(os.environ.get("REDESIGN_XI_SAMPLING", "mixture")).strip().lower()
    if mode == "uniform":
        return stratified_uniform(0.0, theta_max, n)
    # mixture: deep half uniform-θ, neutral half uniform-logξ -> θ
    n_deep = n // 2
    n_neu = n - n_deep
    deep = stratified_uniform(0.0, theta_max, n_deep)
    lx = stratified_uniform(params.get("logξ_min", -3.0), params.get("logξ_max", 5.0), n_neu)
    neutral = tf.exp(-lx)
    return tf.random.shuffle(tf.concat([deep, neutral], axis=0))
...
def sample_state_columns(params, batch_size=None):
    ...
    return (
        ...
        sample_uncertainty_column(params, n),
    )
