"""
PyTorch port of models/params.py.

EXACT 1:1 copy of the numeric calibration dictionary PARAMS and the
MODEL_* environment overrides.  The only behavioural difference is that
``investment_rate_activation`` returns a *torch* callable instead of a
TensorFlow one (same closed-form expression).

Numeric values MUST stay bit-identical to models/params.py.
"""
...
def investment_rate_activation(theta):
    ...
    Torch port of the TF lambda in models/params.py.  Identical algebra:
        f(x) = 1 - (1 + 1/theta) / (exp(2*x) + 1)
    """
    import torch
    ...
    return lambda x: 1.0 - (1.0 + 1.0 / theta) / (torch.exp(2.0 * x) + 1.0)
...
if _model_seed not in (None, ""):
    ...
    import numpy as _np
    _torch.manual_seed(int(_model_seed))
