"""
VALIDATION: torch port of PreDamagePreTech vs the original TF model.

Loads the SAME trained TF checkpoint into both the TF model and the torch port
(this regime's v/i_g/i_d/i_r nets PLUS the frozen neighbour value nets used by
the jump operators), evaluates pde_rhs on the SAME random inputs over the regime
state box, and reports the max RELATIVE discrepancy per quantity.

This regime (π = 1.0 in this run) has the tech jump to PreDamagePostTech and the
damage jump to PostDamagePreTech; the intermediate tech jump is OFF (π == 1).

Run:
    module load python/anaconda-2021.05
    cd /project/lhansen/Cap_damage/TwoStageTechJump_FOCIr_orignal
    python models_torch/validate_pre_damage_pre_tech.py
"""
...
INPUT_DIM = 7  # this regime's own nets take 7-dim X
N = 4096
SEED = 0
...
def main():
    ...
    from PreDamagePreTech import PreDamagePreTechModel as TorchModel
    ...
    ckpt = lambda regime, prefix: os.path.join(RUN_DIR, regime, prefix + "_" + regime)
    ...
    load_tf_weights_into_torch(torch_model.v_nn,   ckpt("PreDamagePreTech", "v_nn_checkpoint"))
    load_tf_weights_into_torch(torch_model.i_g_nn, ckpt("PreDamagePreTech", "i_g_nn_checkpoint"))
    load_tf_weights_into_torch(torch_model.i_d_nn, ckpt("PreDamagePreTech", "i_d_nn_checkpoint"))
    load_tf_weights_into_torch(torch_model.i_r_nn, ckpt("PreDamagePreTech", "i_r_nn_checkpoint"))

    # frozen neighbour value nets (all use the v config; differ only in input dim)
    v_pre_post = TorchNet(v_cfg)
    load_tf_weights_into_torch(v_pre_post, ckpt("PreDamagePostTech", "v_nn_checkpoint"))
    v_post_pre = TorchNet(v_cfg)
    load_tf_weights_into_torch(v_post_pre, ckpt("PostDamagePreTech", "v_nn_checkpoint"))
    torch_model.v_PreDamagePostTech_nn = v_pre_post
    torch_model.v_PostDamagePreTech_nn = v_post_pre
    ...
    # raw NN forwards on this regime's X
    Xnp = np.concatenate([logK, Z, Y, logR, logξ, logξ, logξ], axis=1).astype(np.float32)
    with torch.no_grad():
        ...
        ir_raw_t = nd(torch_model.i_r_nn(torch.from_numpy(Xnp)))
    ...
    for m in ["feedforward_subnet", "params", "pretrained_paths", "PreDamagePreTech"]:
        if m in sys.modules:
            del sys.modules[m]
    sys.path.remove(HERE)  # prefer models/ versions for the TF build
    import tensorflow as tf
    import PreDamagePreTech as TFmod
    ...
    tf_params = {
        ...
        "tensorboard": False,
        "π": 1.0,
        "v_PreDamagePostTech_nn_path": ckpt("PreDamagePostTech", "v_nn_checkpoint"),
        "v_PostDamagePreTech_nn_path": ckpt("PostDamagePreTech", "v_nn_checkpoint"),
        "v_PreDamageIntermTech_nn_path": ckpt("PreDamagePostTech", "v_nn_checkpoint"),  # unused (π==1)
        "export_folder": os.path.join(HERE, "_tf_tmp_predamagepretech"),
    }
    tf_model = TFmod.PreDamagePreTechModel(tf_params)
    tf_model.v_nn.build((None, INPUT_DIM));   tf_model.v_nn.load_weights(ckpt("PreDamagePreTech", "v_nn_checkpoint")).expect_partial()
    tf_model.i_g_nn.build((None, INPUT_DIM)); tf_model.i_g_nn.load_weights(ckpt("PreDamagePreTech", "i_g_nn_checkpoint")).expect_partial()
    tf_model.i_d_nn.build((None, INPUT_DIM)); tf_model.i_d_nn.load_weights(ckpt("PreDamagePreTech", "i_d_nn_checkpoint")).expect_partial()
    tf_model.i_r_nn.build((None, INPUT_DIM)); tf_model.i_r_nn.load_weights(ckpt("PreDamagePreTech", "i_r_nn_checkpoint")).expect_partial()
    ...
    ir_raw_tf = tf_model.i_r_nn(tf.convert_to_tensor(Xnp), training=False).numpy()
    ...
    nn_errs = {
        ...
        "i_r_nn(raw)": max_rel(ir_raw_t, ir_raw_tf),
    }
    ...
    for k in torch_out:
        ...
        pde_errs[k] = e
        print(f"  {k:16s}: rel={e:.3e}  abs={ea:.3e}  "
              f"max|tf|={np.max(np.abs(tf_out[k])):.3e}")
