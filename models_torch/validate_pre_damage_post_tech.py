"""
VALIDATION: torch port of PreDamagePostTech vs the original TF model.
...
This regime adds the TECH-JUMP operator (frozen PostDamagePostTech neighbor net).

Run:
    module load python/anaconda-2021.05
    cd /project/lhansen/Cap_damage/TwoStageTechJump_FOCIr_orignal
    python models_torch/validate_pre_damage_post_tech.py
"""
...
CKPT_DIR = os.path.join(RUN_DIR, "PreDamagePostTech")
NEIGHBOR_CKPT = os.path.join(
    RUN_DIR, "PostDamagePostTech", "v_nn_checkpoint_PostDamagePostTech"
)

MAIN_DIM = 6       # PreDamagePostTech main nets take a 6-dim input
NEIGHBOR_DIM = 7   # frozen PostDamagePostTech neighbor net is a 7-dim net
N = 4096
SEED = 0
...
def main():
    ...
    from PreDamagePostTech import PreDamagePostTechModel as TorchModel
    ...
    load_tf_weights_into_torch(torch_model.v_nn, os.path.join(CKPT_DIR, "v_nn_checkpoint_PreDamagePostTech"))
    load_tf_weights_into_torch(torch_model.i_g_nn, os.path.join(CKPT_DIR, "i_g_nn_checkpoint_PreDamagePostTech"))
    load_tf_weights_into_torch(torch_model.i_d_nn, os.path.join(CKPT_DIR, "i_d_nn_checkpoint_PreDamagePostTech"))
    # Frozen tech-jump neighbor: PostDamagePostTech value net (7-dim input).
    load_tf_weights_into_torch(torch_model.v_PostDamagePostTech_nn, NEIGHBOR_CKPT)
    ...
    # also raw MAIN NN forwards on the assembled 6-dim X
    Xnp = np.concatenate(
        [logK, Z, Y, TORCH_PARAMS["A_g_prime_prime"] * np.ones_like(Y), logξ, logξ],
        axis=1,
    ).astype(np.float32)
    with torch.no_grad():
    ...
    tf_params = {
        ...
        "tensorboard": False,
        "export_folder": os.path.join(HERE, "_tf_tmp_pre_post"),
        # The TF __init__ builds + loads the frozen PostDamagePostTech net from here.
        "v_PostDamagePostTech_nn_path": NEIGHBOR_CKPT,
    }
    tf_model = TFmod.PreDamagePostTechModel(tf_params)
    tf_model.v_nn.build((None, MAIN_DIM)); tf_model.v_nn.load_weights(os.path.join(CKPT_DIR, "v_nn_checkpoint_PreDamagePostTech")).expect_partial()
    tf_model.i_g_nn.build((None, MAIN_DIM)); tf_model.i_g_nn.load_weights(os.path.join(CKPT_DIR, "i_g_nn_checkpoint_PreDamagePostTech")).expect_partial()
    tf_model.i_d_nn.build((None, MAIN_DIM)); tf_model.i_d_nn.load_weights(os.path.join(CKPT_DIR, "i_d_nn_checkpoint_PreDamagePostTech")).expect_partial()

    # raw MAIN NN forwards
    ...
    #  REPORT
    # ============================================================
    print("\n==== MAIN NN forward (torch vs TF) max rel error ====")
    nn_errs = {
    ...
    print(f"  {'quantity':16s}  {'max_rel':>11s}  {'max_abs':>11s}")
    pde_errs = {}
    pde_abs = {}
    for k in torch_out:
        e = max_rel(torch_out[k], tf_out[k])
        a = max_abs(torch_out[k], tf_out[k])
        pde_errs[k] = e
        pde_abs[k] = a
        print(f"  {k:16s}  {e:11.3e}  {a:11.3e}")

    print("\nRESULT_JSON " + repr({"nn": nn_errs, "pde_rel": pde_errs, "pde_abs": pde_abs}))
