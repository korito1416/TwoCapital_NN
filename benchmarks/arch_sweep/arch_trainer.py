...
class PostDamagePostTechModel:
    ...
    def train(self):
        ...
        # SANDBOX: FROM_SCRATCH=1 -> skip NBER + pretrained warm-start (random init), so ANY
        # architecture (width/depth) can be trained from zero to find its loss limit.
        if os.environ.get("FROM_SCRATCH", "0").lower() in {"1", "true", "yes", "on"}:
            NBER_folder = None
        else:
