"""
A/B test: pointwise HJB-residual weight 1/(|mu_Z|+eps) (precond) vs raw (ones).

Decides whether the preconditioning dimension actually lowers TRUE FD error and
the final residual, or only kappa. Adapted from closed_form_control_deinvest_diag.py.

For each seed: build ONE net init, clone its weights, run the SAME budget twice
(precond=True vs precond=False) from the IDENTICAL init. Report final residual
(the loss) and max|i_d - FD| vs FD for both arms, on two paths:
  (A) FD-seed (supervised) + HJB+LBFGS
  (B) scratch + Adam + HJB+LBFGS
"""
import os, sys
import numpy as np
...
def raw_residual_rms(net):
    """Unweighted RMS HJB residual, comparable across arms (precond uses weighted loss)."""
    with tf.GradientTape() as inner:
        inner.watch(Zt); v = value(net, Zt)
    vp = clamp(inner.gradient(v, Zt), Zt)
    q_d = 1 - Zt * vp; q_g = 1 + (1 - Zt) * vp
...
for seed in SEEDS:
    ...
    # ---- Path A: FD-seed + HJB + LBFGS ----
    tf.random.set_seed(seed); np.random.seed(seed)
    net0 = make_net()
    supervised(net0)            # shared supervised warm-start
