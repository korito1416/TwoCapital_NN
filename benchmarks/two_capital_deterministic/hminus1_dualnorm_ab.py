"""
A/B test of the H^-1 (negative-Sobolev) dual-norm residual loss vs the strong-L2
baseline on the two-capital deterministic benchmark NN.

CONTROL  (L2 strong) : loss = mean(R^2)
TREATED  (H^-1 dual) : loss = mean( (Lpi^{-1} R)^2 )
   where Lpi is the FROZEN-CONTROL linearized HJB generator (upwind tridiagonal,
   matching theta_sensitivity/fd_solver assembly), Dirichlet interior ends, rebuilt
   each forward pass with mu frozen at the current NN controls (stop_gradient).
   The tridiagonal solve is differentiable through R only (operator entries frozen),
   i.e. the policy-evaluation linearization.  Pre-applying Lpi^{-1} lifts the flat
   v' transport mode from ~1/mu^2 (L2) toward ~1/mu (H^-1).

SAME seed / init / optimizer budget (Adam + L-BFGS) for both arms.
TRUE ERROR vs FD ground truth only: max|i_d - i_d_FD|, ||v_nn - v_fd||_inf,
max|v'_nn - slope_FD|.  Loss numbers are NOT compared (norms differ).

Two regimes tested: A_d=0.05 (de-invest case) and A_d=0.13.
"""
import os, sys, json
import numpy as np
...
def residual_pieces(net, ctx):
    """Return R (strong residual, [N,1]) and mu (drift, [N,1]) with grad to net."""
    ...
    def lg():
        with tf.GradientTape() as outer:
            ...
            if mode == "l2":
                ...
                # Build frozen-control upwind tridiagonal generator Lpi on interior grid.
                #   (Lpi v)_n = mu_n * (D^up v)_n - delta * v_n
                # with upwind: forward where mu>0, backward where mu<0.
                # We solve Lpi^{-1} R as a single tridiagonal solve, freezing mu.
                muf = tf.stop_gradient(mu[:, 0])           # [N]
                N = tf.shape(muf)[0]
                coef = muf / dZ
                fwd = muf > 0.0
                # diagonal: -delta  (+coef if fwd, -coef if bwd)
                diag = -dl + tf.where(fwd, coef, -coef)
                # super: -coef if fwd else 0 ; sub: 0 if fwd else +coef
                sup = tf.where(fwd, -coef, tf.zeros_like(coef))
                sub = tf.where(fwd, tf.zeros_like(coef), coef)
                # tf tridiagonal_solve expects diagonals [3,N]: superdiag, diag, subdiag
                # row0 = superdiag (sup[i] multiplies x[i+1]); row2 = subdiag (sub[i] multiplies x[i-1])
                sup_sh = tf.concat([sup[:-1], [0.0]], axis=0)
                sub_sh = tf.concat([[0.0], sub[1:]], axis=0)
                diags = tf.stack([sup_sh, diag, sub_sh], axis=0)  # [3,N]
                rhs = tf.reshape(R, [1, N, 1])                    # solve along last dim
                # tridiagonal_solve solves A x = rhs; A is [N,N] from 3-row diagonals
...
def main():
        ...
        for mode in ["l2", "hm1"]:
            ...
            for s in seeds:
                ...
                print(f"  {mode:4s} seed={s}  max|i_d-FD|={e['max_id_err']:.4f}  "
                      ...
                      f"deinvest={e['deinvest']}", flush=True)
    ...
    with open(os.path.join(od, "hminus1_dualnorm_ab.json"), "w") as f:
        json.dump(results, f, indent=2)
    print("\nSAVED outputs/hminus1_dualnorm_ab.json", flush=True)
