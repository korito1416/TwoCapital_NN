...
\title{Two-Capital Adjustment-Cost Models: Settings and Numerical Experiments}

\author{Climate–Innovation–Uncertainty project}

\newdate{articleDate}{25}{6}{2026}
...
\begin{enumerate}
\item \textbf{Deterministic} --- no capital risk (a first-order HJB ODE).
\item \textbf{Capital shocks} --- independent Brownian shocks to each capital stock (a
second-order HJB ODE).
\item \textbf{Model uncertainty} --- Hansen--Sargent robustness on top of the shocks (the
...
investing and \textbf{de-investing} dirty capital.
...
negative even though \textbf{investment may be negative --- de-investment is allowed}).
...
(so $(1 -Z)q_d + Z q_g = 1$), the first-order conditions --- \textbf{identical in all three
models} --- are
...
\subsection*{Calibration (from \texttt{models/params.py})}
...
\section*{2. Model 1 --- Deterministic (no capital risk)}
...
and the HJB collapses to the \textbf{first-order} ODE
...
\subsection*{2.2 FD vs NN benchmark}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{files/det_fd_vs_nn-fb42bdf0addf8adbea42c3d9a92c25e8.png}
\caption[]{Deterministic FD (blue) vs NN (red): $i^d(Z)$, $i^g(Z)$, slope $v'(Z)$, aggregate
productivity $\bar A(Z)=(1 -Z)A_d+ZA_g$, and consumption/output $C/Y$.}
\label{fig-det-fdnn}
\end{figure}

\subsection*{2.3 Adjustment-cost curvature sweep ($\theta$)}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{files/det_theta_Z07-cbb92db0292fef6acb887defac125a80.png}
\caption[]{$\theta$ sweep at $Z=0.7$: investment rates $i^d,i^g$ (left) and the capital-growth
factor $1+\theta i^j\ge0$ (right). Investment can be negative; capital cannot.}
\label{fig-det-theta}
\end{figure}

\subsection*{2.4 Dirty-productivity sweep ($A_d$) --- \textit{new}}

\textbf{Experiment.} Keep the original adjustment cost, fix green at $A_g=0.1567$, and
sweep dirty productivity $A_d$ from 0.01 to 0.13. Read the optimal dirty
investment $i^d$ at three fixed shares $Z=0.6,0.7,0.8$.

\textbf{Result.} $i^d$ rises monotonically with $A_d$ and crosses zero at
$A_d^\star\approx0.073$ (almost independent of $Z$). Below the crossing dirty
capital is unproductive enough that the planner \textbf{de-invests} it
($i^d<0$, down to $\approx -0.038$ at $A_d=0.01$); above it $i^d>0$ (up to
$\approx+0.064$ at $A_d=0.13$). Green $i^g$ stays $\approx0.14$--0.15.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\linewidth]{files/det_ad_combined-9e3d99e49f7d904914e85d39d6d35c56.png}
\caption[]{Dirty investment $i^d$ vs $A_d$ at $Z=0.6,0.7,0.8$ ($A_g=0.1567$ fixed). Zero
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/det_ad_Z06-2d406035ac783c518f7c4a1b5252db0f.png}
\caption[]{$A_d$ sweep, $Z=0.6$.}
\label{fig-det-ad06}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/det_ad_Z07-6d3cd120f8406ee39308321632ac2da6.png}
\caption[]{$A_d$ sweep, $Z=0.7$.}
\label{fig-det-ad07}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/det_ad_Z08-db19b3023bf51b9fe706e0795af06abc.png}
\caption[]{$A_d$ sweep, $Z=0.8$.}
\label{fig-det-ad08}
\end{figure}

\subsection*{2.5 Green-productivity sweep ($A_g$) --- \textit{new, the symmetry question}}

\textbf{Experiment (Lars's follow-up).} Hold dirty fixed at $A_d=0.1303$ and instead
sweep the \textbf{green} productivity $A_g$ --- i.e. ask how large a tech jump in $A_g$ is
needed before the planner \textbf{de-invests dirty} ($i^d<0$). The natural conjecture,
read off Section 2.4, is symmetry: $i^d$ should turn negative once
$A_g\approx 2A_d$.

\textbf{Result --- the response is \textit{not} symmetric.} $i^d$ falls with $A_g$ but only
\textit{slowly}, and does not cross zero until
...
So green productivity must reach roughly \textbf{four times} dirty productivity
($A_g\approx0.5$), not two times, to drive dirty investment negative.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.8\linewidth]{files/det_ag_combined-f987f61320a03dc1282d3ecb7ff93828.png}
\caption[]{Dirty investment $i^d$ vs $A_g$ at $Z=0.6,0.7,0.8$ ($A_d=0.1303$ fixed). $i^d$ does
not cross zero until $A_g\approx0.5\approx 4A_d$ (red line). The vertical dashed line
marks the symmetric point $A_g=A_d$.}
\label{fig-det-ag}
\end{figure}

\textbf{Why the asymmetry.} From the FOC, $i^d = \Gamma c\,q_d/\delta - 1/\theta$, and
since $\Gamma\theta\approx1$, $i^d<0$ requires $c\,q_d \le \delta/(\Gamma\theta)\approx\delta$.
Lowering $A_d$ directly cuts \textit{both} output $c$ \textit{and} dirty's own attractiveness, so
$i^d$ collapses quickly (Section 2.4). Raising $A_g$ instead \textit{raises} output $c$
...
abandons dirty capital readily when dirty is \textit{intrinsically} unproductive, but is
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/det_ag_Z06-d63dcd8dd96b45809162e35ec31ddea1.png}
\caption[]{$A_g$ sweep, $Z=0.6$.}
\label{fig-det-ag06}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/det_ag_Z07-978d3a05ab9420cf00dced9d23d4bbdd.png}
\caption[]{$A_g$ sweep, $Z=0.7$.}
\label{fig-det-ag07}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/det_ag_Z08-e4d0a8ff300c786e61aed3ee77c3cdb5.png}
\caption[]{$A_g$ sweep, $Z=0.8$.}
\label{fig-det-ag08}
\end{figure}
...
\section*{3. Model 2 --- Capital shocks}

\subsection*{3.1 Setting}
...
Itô reduction to $(\log K, Z)$ yields the \textbf{second-order} HJB
...
The active $v''$ term makes the problem elliptic and \textbf{better conditioned} than
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{files/shock_multisigma-34c7c7e124742cb4b6d6c782d9a73a2c.png}
\caption[]{FD solutions at $\sigma\in\{0.01,0.2\}$: investment, marginal values $q_d,q_g$,
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{files/shock_correlation-b685922dc1019329e3b8e43b3c6e3564.png}
\caption[]{Correlated shocks ($\rho=0$ vs 0.9) at $\sigma=0.2$: positive correlation shrinks
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{files/shock_fd_vs_nn-8639228db40116a351719f79ddaa075d.png}
\caption[]{FD vs NN (DGM-PIA with the $v''$ term) at $\sigma=0.01$, forward-net architecture
...
\section*{4. Model 3 --- Model uncertainty (robust control)}
...
and adds a \textbf{robustness drag} to the second-order HJB:
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{files/unc_fd_sigma001-00e3e68d7986845d807469399db74521.png}
\caption[]{Robust FD across $\xi\in\{148.4,0.1,0.05\}$ at $\sigma=0.01$ (calibration):
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{files/unc_fd_sigma02-e1d4d720e31c21f785bc522be152617a.png}
\caption[]{Same at $\sigma=0.2$: the robustness drag is now visible --- stronger uncertainty
...
\begin{itemize}
\item \textbf{One framework, three layers.} The state reduction and FOCs are common; risk
...
\item \textbf{De-investment is governed by \textit{own} productivity.} Dirty investment turns
...
productivity. The model is therefore \textbf{far from symmetric} in $(A_d,A_g)$: a
large green tech jump shifts \textit{new} investment toward green and lowers $i^d$, but
does not by itself drive the planner to liquidate a still-productive dirty stock.
\item \textbf{NN validation.} In all three models the three-loss DGM-PIA NN matches the FD
